...
const Sidebar = ({ activeTab, setActiveTab, mobileOpen, setMobileOpen, onLogout }) => {
  ...
  const navStructure = [
    // 1. Admin / General Modules (Kept at top for standard access)
    {
      ...
      forms: [{ id: 'donations-list', label: 'All Donations (Offline)' }]
    },
    {
      id: 'general',
      title: 'General',
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 z-30 h-screen w-64 bg-[#0B1120] border-r border-slate-800 transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} overflow-hidden flex flex-col shadow-2xl`}>
        ...
        <nav className="flex-1 overflow-y-auto py-4 custom-scrollbar">
          <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 mb-4 text-sm font-medium transition-colors ${activeTab === 'dashboard' ? 'bg-amber-500 text-[#0B1120]' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}><LayoutDashboard className="w-5 h-5" />Dashboard (Offline)</button>
