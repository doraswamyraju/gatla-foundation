...
const EducationMemberForm = ({ onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const apiUrl = isLocal ? 'http://localhost/gatla-foundation/api/submit_education_member.php' : 'https://gatlafoundation.org/api/submit_education_member.php';
    ...
    return (
        <div className="flex flex-col bg-[#0B1120] text-white h-full">
            <div className="px-8 py-6 border-b border-slate-800"><h2 className="text-2xl font-bold text-green-500">Education Club Member Form</h2></div>
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
                ...
                <button type="submit" disabled={loading} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2">{loading ? <Loader2 className="animate-spin" /> : 'Register Member'}</button>
            </form>
        </div>
    );
};
export default EducationMemberForm;
