...
const CricketDonorForm = ({ onClose }) => {
    ...
    const handlePayment = async (e) => {
        ...
        const options = {
            ...
            name: "Gatla Cricket", description: "Cricket Support",
            ...
            theme: { color: "#3B82F6" } // Blue for Cricket
        };
        new window.Razorpay(options).open();
    };

    if (success) return <div className="p-12 text-center text-white"><CheckCircle2 className="w-16 h-16 text-blue-500 mx-auto mb-4" /><h3>Donation Successful!</h3></div>;

    return (
        <div className="flex flex-col bg-[#0B1120] text-white h-full">
            <div className="px-8 py-6 border-b border-slate-800"><h2 className="text-2xl font-bold text-blue-500">Cricket Donor</h2></div>
            <form onSubmit={handlePayment} className="flex-1 overflow-y-auto p-8 space-y-6">

                <div className="grid grid-cols-3 gap-3">
                    {[500, 1000, 2000, 5000].map(amt => (
                        <button key={amt} type="button" onClick={() => { setAmount(amt); setCustomAmount(''); }} className={`py-2 rounded border ${amount === amt && !customAmount ? 'bg-blue-600 border-blue-600' : 'border-slate-700'}`}>₹{amt}</button>
                    ))}
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded text-blue-400 text-sm font-bold text-center">
                    ♥ Supporting Gatla Cricket Club
                </div>
                ...
                <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2">
...
export default CricketDonorForm;
