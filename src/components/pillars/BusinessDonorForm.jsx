...
const BusinessDonorForm = ({ onClose }) => {
    ...
    const handlePayment = async (e) => {
        ...
        const options = {
            ...
            name: "Gatla Business", description: "Business Support",
    ...
    if (success) return <div className="p-12 text-center text-white"><CheckCircle2 className="w-16 h-16 text-amber-500 mx-auto mb-4" /><h3>Donation Successful!</h3></div>;

    return (
        <div className="flex flex-col bg-[#0B1120] text-white h-full">
            <div className="px-8 py-6 border-b border-slate-800"><h2 className="text-2xl font-bold text-amber-500">Business Donor</h2></div>
            <form onSubmit={handlePayment} className="flex-1 overflow-y-auto p-8 space-y-6">

                <div className="grid grid-cols-3 gap-3">
                    {[500, 1000, 2000, 5000].map(amt => (
                        <button key={amt} type="button" onClick={() => { setAmount(amt); setCustomAmount(''); }} className={`py-2 rounded border ${amount === amt && !customAmount ? 'bg-amber-600 border-amber-600' : 'border-slate-700'}`}>₹{amt}</button>
                    ))}
                ...
                <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded text-amber-400 text-sm font-bold text-center">
                    ♥ Supporting Gatla Business Club
                </div>
                ...
                <button type="submit" disabled={loading} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2">
...
export default BusinessDonorForm;
