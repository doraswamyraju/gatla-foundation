...
const EducationDonorForm = ({ onClose }) => {
      ...
      <form onSubmit={handlePayment} className="flex-1 overflow-y-auto p-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[500, 1000, 2000, 5000].map(amt => (
            <button key={amt} type="button" onClick={() => { setAmount(amt); setCustomAmount(''); }} className={`py-2 rounded border ${amount === amt && !customAmount ? 'bg-green-600 border-green-600' : 'border-slate-700'}`}>₹{amt}</button>
          ))}
        ...
        {/* Custom Field */}
        <div className="bg-green-500/10 border border-green-500/20 p-3 rounded text-green-400 text-sm font-bold text-center">
          ♥ Supporting Gatla Education Club
        </div>
