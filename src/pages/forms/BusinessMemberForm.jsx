...
const BusinessMemberForm = ({ onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/submit_business_member.php`, {
                method: 'POST',
                body: payload
            });
            const result = await response.json();
            if (result.status === 'success') {
                alert("Business Member Registration Successful!");
                if (onClose) onClose();
            } else {
            ...
            <div className="p-6 border-b border-slate-800 flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                        <Briefcase className="w-5 h-5 text-red-500" /> Business Member Registration
                    </h2>
                    <p className="text-xs text-slate-400 mt-1">Join the network of entrepreneurs</p>
                </div>
            </div>
            <div className="p-6">
