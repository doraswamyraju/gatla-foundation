...
const PillarFormWrapper = ({ title, description, children, accentColor = 'text-green-500', onClose, formData, formType }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // This ensures PHP $_POST works correctly and allows for file uploads if needed later.
        const dataToSend = new FormData();
        
        // Add the form type
        dataToSend.append('form_type', formType);

        // Map React state keys to Backend Database keys (snake_case)
        // We use || '' to ensure we don't send 'undefined' strings
        dataToSend.append('full_name', formData.fullName || '');
        dataToSend.append('father_name', formData.fatherName || '');
        dataToSend.append('address', formData.fullAddress || '');
        dataToSend.append('phone_no', formData.phoneNo || '');
        dataToSend.append('email_id', formData.emailId || '');
        dataToSend.append('aadhaar_no', formData.aadharNo || '');
        dataToSend.append('pan_card_no', formData.panCardNo || '');
        dataToSend.append('qualification', formData.qualification || '');
        dataToSend.append('occupation', formData.occupation || '');
        dataToSend.append('areas_of_interest', formData.areasOfInterest || '');
        dataToSend.append('support_type', formData.supportThrough || '');
        ...
        try {
            ...
            const response = await fetch(API_URL, { 
                method: 'POST',
                // FIX: Do NOT set Content-Type to application/json when using FormData
