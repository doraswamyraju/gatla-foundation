...
const CricketMemberForm = ({ onClose, initialData, onSaveSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white">
      <div className="grid grid-cols-2 gap-4">
        <input name="full_name" value={formData.full_name} onChange={handleChange} placeholder="Member Name" className="border p-2 rounded" required />
      ...
      <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 text-white py-2 rounded flex justify-center gap-2">
        {isSubmitting ? <Loader2 className="animate-spin" /> : <Save />} Save Member
      </button>
    </form>
  );
};
